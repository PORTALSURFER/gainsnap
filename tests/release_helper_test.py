#!/usr/bin/env python3
"""Focused regression tests for GainSnap's release identity and artifact contracts."""
...
SOURCE_SHA = "a" * 40
BUILD_ID = f"gainsnap-v0.1.0-nightly.17-{SOURCE_SHA[:12]}"
...
def write_archives(root: Path, publication_version: str, *, windows: bool) -> tuple[Path, Path, Path | None]:
    clap = root / f"gainsnap-v{publication_version}-macos.clap.zip"
    vst3 = root / f"gainsnap-v{publication_version}-macos.vst3.zip"
    clap.write_bytes(b"macOS CLAP")
    vst3.write_bytes(b"macOS VST3")
    windows_archive = None
    if windows:
        windows_archive = root / f"gainsnap-v{publication_version}-windows-x86_64-unsigned.vst3.zip"
...
class ReleaseHelperTests(unittest.TestCase):
    def test_nightly_publication_version_uses_package_version_and_workflow_sequence(self) -> None:
        self.assertEqual(
            release_helper.derive_publication_version("0.1.0", "nightly", 17),
            "0.1.0-nightly.17",
        )
        self.assertEqual(
            release_helper.derive_publication_version("0.1.0", "stable", 17),
            "0.1.0",
        )
        with self.assertRaisesRegex(ValueError, "positive canonical"):
            release_helper.derive_publication_version("0.1.0", "nightly", 0)
        with self.assertRaises(ValueError):
            release_helper.validate_publication_version(
                "0.1.0", "0.2.7-nightly.17", "nightly"
            )
            ...
            "releases": [
                {
                    ...
                    "source": {"repository": "PORTALSURFER/gainsnap", "git_sha": source},
                }
            ]
        }
        self.assertIsNone(
    ...
    def test_nightly_release_decision_only_noops_for_exact_publication_identity(self) -> None:
        ...
        build_id = f"gainsnap-v{publication}-{source[:12]}"
        document = {
            "releases": [
                {
                    "channel": "nightly",
        ...
        with self.assertRaisesRegex(ValueError, "must advance"):
            release_helper.nightly_release_decision(
                ...
                build_id=f"gainsnap-v0.2.7-nightly.75-{source[:12]}",
                source_sha=source,
                document=document,
            )
        ...
        for channel in ("stable", "rc"):
            with self.subTest(channel=channel), tempfile.TemporaryDirectory() as directory:
                ...
                manifest = build_release(
                    root,
                    channel=channel,
                    publication_version=("0.1.0" if channel == "stable" else "0.1.0-rc.17"),
                    distribution="production",
                    windows=False,
                    build_id=f"gainsnap-v{('0.1.0' if channel == 'stable' else '0.1.0-rc.17')}-{SOURCE_SHA[:12]}",
                )
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                ...
                [
                    ...
                    "gainsnap-v0.1.0-nightly.17-windows-x86_64-unsigned.vst3.zip",
                ],
            )
            self.assertEqual(
    ...
    def test_schema2_uploads_only_four_files_and_schema3_is_node_only(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            manifest = build_release(
                ...
                distribution="production",
                windows=False,
                build_id=f"gainsnap-v0.1.0-{SOURCE_SHA[:12]}",
            )
    ...
    def test_release_and_workflow_keep_shared_identity_without_version_bump(self) -> None:
        ...
        self.assertIn("validate_preflight_manifest", release_script)
        self.assertIn('python3 - "${archive}"', release_script)
        self.assertIn("gainsnap-v${publication_version}-windows-x86_64-unsigned.vst3.zip", release_script)
        for forbidden in (
    ...
    def test_windows_and_preflight_workflows_preserve_security_boundaries(self) -> None:
        ...
        self.assertIn("dist/gainsnap-v${PACKAGE_VERSION}.vst3/Contents/x86_64-win/gainsnap-v${PACKAGE_VERSION}.vst3", windows)
        self.assertIn("gainsnap-v${PUBLICATION_VERSION}-windows-x86_64-unsigned.vst3.zip", windows)
    ...
    def test_nightly_scheduler_is_main_only_and_has_no_release_credentials(self) -> None:
        ...
        self.assertIn("github.repository == 'PORTALSURFER/gainsnap'", workflow)
    ...
    def test_public_catalog_describes_schema3_windows_nightlies(self) -> None:
        project = Path(__file__).parents[1]
        product = json.loads((project / "site" / "product.json").read_text(encoding="utf-8"))
        distribution = product["distribution"]
        self.assertEqual(distribution["release_manifest_schema"], 3)
        self.assertEqual(distribution["channels"], ["stable", "rc", "nightly"])
        profile = distribution["release_profile"]
        self.assertEqual(profile["apple_team_id"], "DKTKQ8U5T8")
        self.assertEqual(
            {(target["platform"], target["architectures"][0], target["format"], target["name_template"]) for target in profile["artifact_targets"]},
            {
                ("macos", "arm64", "clap", "gainsnap-v{version}-macos.clap.zip"),
                ("macos", "arm64", "vst3", "gainsnap-v{version}-macos.vst3.zip"),
                ("windows", "x86_64", "vst3", "gainsnap-v{version}-windows-x86_64-unsigned.vst3.zip"),
            },
        )
