#!/usr/bin/env python3
"""Build and validate gainsnap's unsigned Windows VST3 release artifact."""
...
SAFE_ARCHIVE_NAME = re.compile(
    r"^gainsnap-v[0-9A-Za-z.-]+-windows-x86_64-unsigned\.vst3\.zip$"
)
...
def bundle_name(package_version: str) -> str:
    return f"gainsnap-v{package_version}.vst3"


def archive_name(publication_version: str) -> str:
    return f"gainsnap-v{publication_version}-windows-x86_64-unsigned.vst3.zip"
...
def _validate_source_binary_path(binary: Path, package_version: str) -> None:
    ...
    if tuple(binary.parts[-len(expected_suffix) :]) != expected_suffix:
        expected = Path(*expected_suffix)
        raise ValueError(f"Windows VST3 build output must be under {expected}")
...
def _validate_zip_member_path(info: zipfile.ZipInfo) -> None:
    ...
    if not parts or any(part in {"", ".", ".."} for part in name.split("/")):
        raise ValueError(f"archive member path is unsafe: {name!r}")
    if info.is_dir():
        raise ValueError("archive must not contain directory entries")
...
def validate_archive(
    ...
    *,
    ...
    if not SHA256.fullmatch(archive_hash) or not SHA256.fullmatch(member_hash):
        raise ValueError("Windows artifact hash is invalid")
    return {
        "name": archive.name,
        "sha256": archive_hash,
        "size_bytes": archive_size,
        "bundle_name": bundle_name(package_version),
...
def _parse_git_source(source: Any, package_name: str) -> dict[str, str]:
    ...
    if not separator or not isinstance(revision, str) or SHA1.fullmatch(revision) is None:
    ...
    if query_separator:
        ...
        if "rev" in query_values and query_values["rev"] != revision:
            raise ValueError(f"{package_name} dependency git query and revision disagree")
    if not repository.startswith("https://"):
        raise ValueError(f"{package_name} dependency repository must use HTTPS")
...
def load_dependency_revisions(lockfile: Path) -> dict[str, dict[str, str]]:
    ...
    for name in ("toybox", "radiant"):
        ...
        dependency = _parse_git_source(candidates[0].get("source"), name)
...
def _parse_lock_packages_without_tomllib(contents: str) -> list[dict[str, Any]]:
    """Read the package name/source fields needed on Python versions < 3.11."""
...
def _validate_dependencies(dependencies: Mapping[str, Any]) -> dict[str, dict[str, str]]:
    if not isinstance(dependencies, Mapping):
        raise ValueError("manifest dependency revisions are invalid")
    ...
    for name in sorted(REQUIRED_DEPENDENCIES):
        ...
        if not isinstance(value, dict) or set(value) != {"repository", "revision"}:
            raise ValueError(f"manifest dependency metadata is invalid: {name}")
...
def dependency_revisions(lockfile: Path, *, vst3_sdk_revision: str = VST3_SDK_REVISION) -> dict[str, dict[str, str]]:
    ...
    dependencies["vst3sdk"] = {
        "repository": VST3_SDK_REPOSITORY,
        "revision": vst3_sdk_revision,
    }
    ...
    *,
    ...
    return {
        ...
        "archive": {
            ...
            "layout": {
                "bundle": details["bundle_name"],
                "binary": details["member_name"],
            },
            "members": [
                {
                    "path": details["member_name"],
                    "sha256": details["member_sha256"],
                    "size_bytes": details["member_size_bytes"],
                }
            ],
        },
    }
...
def validate_manifest(
    ...
    *,
    ...
    vst3_sdk_revision: str = VST3_SDK_REVISION,
) -> None:
    if not isinstance(manifest, Mapping):
        raise ValueError("Windows artifact manifest must be an object")
    required = {
        "schema_version",
        "product",
        "format",
    ...
    if set(manifest) != required or type(manifest.get("schema_version")) is not int or manifest.get("schema_version") != MANIFEST_SCHEMA:
    ...
    if not isinstance(source, dict) or set(source) != {"repository", "git_sha", "dirty"}:
    ...
    if cargo_lock is not None:
        expected = dependency_revisions(cargo_lock, vst3_sdk_revision=vst3_sdk_revision)
    ...
    if archive["media_type"] != "application/zip" or type(archive["member_count"]) is not int or archive["member_count"] != 1 or archive["path_policy"] != WINDOWS_PATH_POLICY:
        raise ValueError("Windows artifact archive policy is invalid")
    ...
    if not isinstance(archive["layout"], dict) or set(archive["layout"]) != {"bundle", "binary"}:
    ...
    if archive["layout"] != {"bundle": expected_bundle, "binary": expected_member}:
        raise ValueError("Windows artifact archive layout metadata is invalid")
    members = archive["members"]
    if not isinstance(members, list) or len(members) != 1 or not isinstance(members[0], dict) or set(members[0]) != {"path", "sha256", "size_bytes"}:
        raise ValueError("Windows artifact archive member metadata is invalid")
    member = members[0]
    if member["path"] != expected_member or not isinstance(member["sha256"], str) or not SHA256.fullmatch(member["sha256"]) or not _positive_int(member["size_bytes"]):
        raise ValueError("Windows artifact archive member metadata is invalid")
    details = validate_archive(
        expected_archive,
        package_version=manifest["package_version"],
    ...
    if (
        details["sha256"] != archive["sha256"]
        or details["size_bytes"] != archive["size_bytes"]
        or details["member_sha256"] != member["sha256"]
        or details["member_size_bytes"] != member["size_bytes"]
    ):
...
def _package_command(args: argparse.Namespace) -> None:
    dependencies = dependency_revisions(Path(args.cargo_lock), vst3_sdk_revision=args.vst3_sdk_revision)
    manifest = package_windows_vst3(
...
def _validate_command(args: argparse.Namespace) -> None:
    ...
    if not isinstance(manifest, dict):
        raise ValueError("Windows artifact manifest must be a JSON object")
    validate_manifest(
        manifest,
        root,
        cargo_lock=Path(args.cargo_lock),
        vst3_sdk_revision=args.vst3_sdk_revision,
    )
