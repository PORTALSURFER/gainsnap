#!/usr/bin/env python3
"""Validate the GainSnap nightly artifact and publisher-integration contracts."""
...
TEST_RELEASE_TOKEN = "gainsnap-integration-release-token"
TEST_OIDC_REQUEST_TOKEN = "gainsnap-integration-oidc-request-token"
TEST_ATTESTATION_TOKEN = "gainsnap-integration-attestation-token"
TEST_NOTARY_ID = "00000000-0000-4000-8000-000000000000"
CAPABILITY_PATH = "/plugins/api/v1/products/gainsnap/releases"
...
def _require_regular_file(path: Path, label: str) -> None:
    if path.is_symlink() or not path.is_file():
        raise HarnessError(f"{label} is not a regular file: {path.name}")


def _find_unique_file(root: Path, name: str, label: str) -> Path:
    require(root.is_dir() and not root.is_symlink(), f"{label} root is not a directory")
    matches = sorted(root.rglob(name))
    require(len(matches) == 1, f"{label} must contain exactly one {name}")
    _require_regular_file(matches[0], label)
    return matches[0]
...
def _require_exact_entries(root: Path, names: set[str], label: str) -> None:
...
def _require_shared_source(manifest: dict[str, Any], source_sha: str, label: str) -> None:
    source = manifest.get("source")
    require(isinstance(source, dict), f"{label} source metadata is invalid")
    require(source.get("repository") == "PORTALSURFER/gainsnap", f"{label} repository is invalid")
...
def _load_macos_preflight(
    ...
    *,
    ...
    manifest_path = _find_unique_file(artifact_root, "release-manifest.json", "macOS preflight artifact")
    root = manifest_path.parent
    manifest = _load_json(manifest_path, "macOS preflight manifest")
    try:
        release_helper.validate_manifest(manifest, root, package_version=package_version)
    ...
    require(manifest.get("product") == "gainsnap", "macOS preflight product is invalid")
    ...
    require(manifest.get("version") == package_version, "macOS preflight package version does not match prepare")
    ...
    _require_shared_source(manifest, source_sha, "macOS preflight")

    artifacts = manifest.get("artifacts")
    screenshot = manifest.get("screenshot")
    changelog = manifest.get("changelog")
    require(isinstance(artifacts, list) and len(artifacts) == 2, "macOS preflight must contain exactly two archives")
    require(isinstance(screenshot, dict), "macOS preflight screenshot metadata is invalid")
    require(isinstance(changelog, dict), "macOS preflight changelog metadata is invalid")
    artifact_names = {
        artifact.get("format"): artifact.get("name")
        for artifact in artifacts
        if isinstance(artifact, dict)
    }
    screenshot_name = screenshot.get("name")
    changelog_name = changelog.get("name")
    require(set(artifact_names) == {"clap", "vst3"}, "macOS preflight archive metadata is invalid")
    clap_name = artifact_names["clap"]
    vst3_name = artifact_names["vst3"]
    require(isinstance(clap_name, str) and isinstance(vst3_name, str), "macOS preflight archive metadata is invalid")
    require(isinstance(screenshot_name, str), "macOS preflight screenshot metadata is invalid")
    require(changelog_name == "CHANGELOG.md", "macOS preflight changelog metadata is invalid")
    require(clap_name == f"gainsnap-v{package_version}-macos.clap.zip", "macOS preflight CLAP archive name is invalid")
    require(vst3_name == f"gainsnap-v{package_version}-macos.vst3.zip", "macOS preflight VST3 archive name is invalid")
    require(len(list(root.glob("*-macos.clap.zip"))) == 1, "macOS preflight must have exactly one CLAP archive")
    require(len(list(root.glob("*-macos.vst3.zip"))) == 1, "macOS preflight must have exactly one VST3 archive")
    require(len(list(root.glob("gainsnap-default-*.png"))) == 1, "macOS preflight must have exactly one screenshot")
    require(len(list(root.glob("CHANGELOG.md"))) == 1, "macOS preflight must have exactly one changelog")
    _require_exact_entries(
        root,
        {"release-manifest.json", clap_name, vst3_name, screenshot_name, changelog_name},
...
def _load_windows_artifact(
    ...
    *,
    ...
    manifest_path = _find_unique_file(artifact_root, "windows-artifact-manifest.json", "Windows artifact")
    root = manifest_path.parent
    ...
    require(manifest.get("schema_version") == windows_release_helper.MANIFEST_SCHEMA, "Windows artifact schema is invalid")
    require(manifest.get("product") == "gainsnap", "Windows artifact product is invalid")
    ...
    _require_shared_source(manifest, source_sha, "Windows artifact")

    archive = manifest.get("archive")
    require(isinstance(archive, dict) and isinstance(archive.get("name"), str), "Windows archive metadata is invalid")
    archive_name = archive["name"]
    require(
        archive_name == windows_release_helper.archive_name(publication_version),
        "Windows archive name does not match prepare",
    )
    _require_exact_entries(root, {"windows-artifact-manifest.json", archive_name}, "Windows artifact")
    return manifest, root, root / archive_name


def _copy_referenced(source: Path, destination: Path, label: str) -> None:
    _require_regular_file(source, label)
    require(not destination.exists(), f"refusing to overwrite {destination.name}")
...
class ApiMock:
    ...
    def _release_upload_prefix(self) -> str:
        ...
        return f"/plugins/api/v1/products/gainsnap/release-uploads/{build_id}"
    ...
    def _stage(
        ...
        _require_regular_file(file_path, "staged release file")
...
def _start_server(mock: Any) -> tuple[StrictServer, threading.Thread, str]:
    ...
    thread = threading.Thread(target=server.serve_forever, name="release-integration-mock", daemon=True)
    thread.start()
...
def _verify_publisher_pin(publisher: Path) -> None:
    ...
    require(
        publisher == publisher_repo / "scripts" / "publish-plugin-release.mjs",
        "publisher script is outside the pinned checkout path",
    )
    try:
        result = subprocess.run(
    ...
    require(
        result.returncode == 0 and result.stdout.strip() == PUBLISHER_COMMIT,
        "publisher checkout is not at the required PortalSurfer commit",
    )
...
def _assembled_release(args: argparse.Namespace) -> Iterator[tuple[dict[str, Any], Path]]:
    ...
    _, _, mac_clap_archive, mac_vst3_archive, mac_screenshot, mac_changelog = _load_macos_preflight(
    ...
    with tempfile.TemporaryDirectory(prefix="gainsnap-release-integration-", dir=scratch_parent) as directory:
        scratch = Path(directory)
        mac_clap_destination = scratch / f"gainsnap-v{args.publication_version}-macos.clap.zip"
        mac_vst3_destination = scratch / f"gainsnap-v{args.publication_version}-macos.vst3.zip"
        windows_archive_destination = scratch / windows_archive.name
        screenshot_destination = scratch / mac_screenshot.name
        changelog_destination = scratch / "CHANGELOG.md"
        _copy_referenced(mac_clap_archive, mac_clap_destination, "macOS preflight CLAP archive")
        _copy_referenced(mac_vst3_archive, mac_vst3_destination, "macOS preflight VST3 archive")
        _copy_referenced(windows_archive, windows_archive_destination, "Windows archive")
        _copy_referenced(mac_screenshot, screenshot_destination, "macOS preflight screenshot")
        _copy_referenced(mac_changelog, changelog_destination, "macOS preflight changelog")

        # The preflight archive is intentionally ad hoc. Fixed test-only signing
        # evidence exercises the production schema-3 shape without claiming a
        # production signing or notarization result.
        manifest = release_helper.build_manifest(
            version=args.publication_version,
            package_version=args.package_version,
            build_id=args.build_id,
            channel="nightly",
            released_at=args.released_at,
            git_sha=args.source_sha,
            clap=mac_clap_destination,
            vst3=mac_vst3_destination,
            screenshot=screenshot_destination,
            changelog=changelog_destination,
            distribution="production",
            signing_team_id="TEAM123456",
            notary_submissions={"clap": TEST_NOTARY_ID, "vst3": TEST_NOTARY_ID},
            windows_vst3=windows_archive_destination,
        )
        manifest_bytes = release_helper.canonical_json(manifest)
        (scratch / "release-manifest.json").write_bytes(manifest_bytes)
        release_helper.validate_manifest(manifest, scratch, package_version=args.package_version)
        _require_exact_entries(
            scratch,
            {
                mac_clap_destination.name,
                mac_vst3_destination.name,
                windows_archive_destination.name,
                screenshot_destination.name,
                changelog_destination.name,
...
def run_artifact_contract(args: argparse.Namespace) -> None:
    with _assembled_release(args) as (manifest, scratch):
        ...
        require(
            ...
            "schema 3 macOS VST3 security metadata is invalid",
        )
        require(
            artifacts[2]["security"] == {"status": "unsigned", "certificate": None},
...
def run_publisher_integration(args: argparse.Namespace) -> None:
    ...
    with _assembled_release(args) as (manifest, scratch):
        ...
        manifest_bytes = release_helper.canonical_json(manifest)
        audience = f"{api_endpoint}/plugins/api/v1/products/gainsnap/release-attestations/sha256/{hashlib.sha256(manifest_bytes).hexdigest()}"
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--scratch-parent", help="Parent for the temporary non-release assembly directory")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    if args.mode == "artifact-contract":
        print("artifact contract ok: real macOS preflight + Windows sidecar assembled schema 3 with an exact six-file scratch set.")
    else:
