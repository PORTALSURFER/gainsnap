...
def pe_image(
    ...
    data[0x1F0:] = b"GainSnap Windows VST3 test payload"
    return bytes(data)
...
class WindowsReleaseHelperTests(unittest.TestCase):
    def package(self, root: Path, *, binary_data: bytes | None = None, output_name: str = "release") -> tuple[dict, Path, Path]:
        binary = root / "dist" / "gainsnap-v0.1.0.vst3" / "Contents" / "x86_64-win" / "gainsnap-v0.1.0.vst3"
        ...
        manifest = windows_release_helper.package_windows_vst3(
            ...
            channel="nightly",
            build_id="gainsnap-v0.1.0-nightly.7-windows-a1b2c3d4e5f6",
    ...
    def test_package_emits_requested_names_layout_and_unsigned_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            archive_path = output / "gainsnap-v0.1.0-nightly.7-windows-x86_64-unsigned.vst3.zip"
            ...
            self.assertEqual(
                ...
                {
                    "bundle": "gainsnap-v0.1.0.vst3",
                    "binary": "gainsnap-v0.1.0.vst3/Contents/x86_64-win/gainsnap-v0.1.0.vst3",
                },
            )
    ...
    def test_package_and_manifest_are_deterministic(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                (first_output / "gainsnap-v0.1.0-nightly.7-windows-x86_64-unsigned.vst3.zip").read_bytes(),
                (second_output / "gainsnap-v0.1.0-nightly.7-windows-x86_64-unsigned.vst3.zip").read_bytes(),
            )
            self.assertEqual(
    ...
    def test_archive_rejects_path_traversal_and_extra_members(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            archive = root / "gainsnap-v0.1.0-nightly.7-windows-x86_64-unsigned.vst3.zip"
    ...
    def test_windows_workflow_pins_and_forwards_build_provenance(self) -> None:
        ...
        self.assertIn("name: gainsnap-windows-${{ steps.metadata.outputs.build_id }}", workflow)
        self.assertIn("name: gainsnap-${{ steps.metadata.outputs.publication_version }}-windows-x86_64-unsigned", workflow)
