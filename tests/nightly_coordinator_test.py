#!/usr/bin/env python3
"""Mocked state-machine tests for the protected GainSnap nightly coordinator."""
        ...
        "releases": [
            {
                "channel": "nightly",
                "version": version,
                "build_id": f"gainsnap-v{version}-{source_sha[:12]}",
...
class CoordinatorTests(unittest.TestCase):
                ...
                document["releases"].append(
                    {
                        "channel": "nightly",
                        "version": publication,
                        "build_id": f"gainsnap-v{publication}-{MERGED_SHA[:12]}",
                ...
                document["releases"].append(
                    {
                        "channel": "nightly",
                        "version": publication,
                        "build_id": f"gainsnap-v{publication}-{MAIN_SHA[:12]}",
                        "released_at": "2026-09-09T09:10:00Z",
    ...
    def test_github_adapter_requires_product_repository_and_release_inputs(self) -> None:
        with self.assertRaisesRegex(coordinator.CoordinatorError, "repository"):
            coordinator.GitHubApi(api_url="https://api.github.com", repository="fork/gainsnap", token="x")
...
class LocalVersionBranchTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.temporary = tempfile.TemporaryDirectory(prefix="gainsnap-nightly-git-")
    ...
    def test_tampered_stale_branch_cannot_be_cleaned_after_merge(self) -> None:
        ...
        self.git(
            ...
            "commit",
            "-m",
            "chore(release): prepare GainSnap v0.2.7 nightly",
            "-m",
            f"{coordinator.VERSION_PR_MARKER}\n{coordinator.VERSION_METADATA_PREFIX}0.2.7",
        )
        self.git(self.seed, "push", "origin", "main")
