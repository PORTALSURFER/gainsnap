...
The vertical meter shows the latest block peaks in realtime: the orange column
is the incoming (IN) level, and the cyan column is the adjusted (OUT) level. A
small triangle beside the meter marks the target and acts as the target control;
the compact interface uses the meter instead of separate numeric IN and OUT
readouts.
