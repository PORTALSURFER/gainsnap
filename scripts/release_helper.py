#!/usr/bin/env python3
"""Dependency-free manifest and PortalSurfer transport helpers for GainSnap."""
...
def validate_publication_version(package_version: str, publication_version: str, channel: str) -> None:
    ...
    if channel == "stable" and publication_version != package_version:
...
def derive_publication_version(package_version: str, channel: str, sequence: Any) -> str:
    ...
    if channel in ("rc", "nightly"):
        publication_version = f"{package_text}-{channel}.{_canonical_nightly_number(sequence)}"
    else:
...
def format_manifest_version(package_version: str, channel: str, nightly_number: Any = None) -> str:
    ...
    if channel == "stable":
        return package_text
    return f"{package_text}-{channel}.{_canonical_nightly_number(nightly_number)}"
...
def nightly_release_decision(
    *,
    ...
    if not isinstance(source_sha, str) or not GIT_SHA.fullmatch(source_sha):
        raise ValueError("source SHA is invalid")
    expected_build_id = f"gainsnap-v{publication_version}-{source_sha[:12]}"
...
def validate_png(path: Path, width: int = SCREENSHOT_WIDTH, height: int = SCREENSHOT_HEIGHT) -> dict[str, Any]:
    """Validate the structural PNG contract used by GainSnap's default capture."""
...
def _screenshot_metadata(screenshot: Path, git_sha: str) -> dict[str, Any]:
    if screenshot.name != SCREENSHOT_FILENAME or not SCREENSHOT_NAME.fullmatch(screenshot.name):
        raise ValueError(f"screenshot name must be {SCREENSHOT_FILENAME}")
...
def _schema2_artifact(*, format_name: str, path: Path, version: str, channel: str) -> dict[str, Any]:
    expected_name = f"gainsnap-v{version}-macos.{format_name}.zip"
...
def _build_schema3_manifest(
    *,
    ...
    expected_build_id = f"gainsnap-v{version}-{git_sha[:12]}"
    ...
    expected_names = {
        "clap": f"gainsnap-v{version}-macos.clap.zip",
        "vst3": f"gainsnap-v{version}-macos.vst3.zip",
        "windows": f"gainsnap-v{version}-windows-x86_64-unsigned.vst3.zip",
    }
...
def build_manifest(
    *,
    ...
    if product != PRODUCT or repository != REPOSITORY:
        raise ValueError("unknown or mismatched Portal product")
    if notary_submissions is not None and (clap_notary_id is not None or vst3_notary_id is not None):
        raise ValueError("provide notary_submissions or the legacy notary IDs, not both")
    if notary_submissions is None and (clap_notary_id is not None or vst3_notary_id is not None):
        if clap_notary_id is None or vst3_notary_id is None:
            raise ValueError("both legacy CLAP and VST3 notary IDs are required")
    ...
    if channel != "nightly" or distribution != "production":
        raise ValueError("schema 3 is only available for production GainSnap nightlies")
...
def _validate_schema2_manifest(
    ...
    *,
    ...
    if formats != {"clap", "vst3"} or not all(isinstance(artifact, dict) for artifact in artifacts):
        raise ValueError("publish requires exactly CLAP and VST3 artifacts")
    expected_names = {
        "clap": f"gainsnap-v{manifest['version']}-macos.clap.zip",
        "vst3": f"gainsnap-v{manifest['version']}-macos.vst3.zip",
    }
    ...
    if (
        ...
        or screenshot["name"] != SCREENSHOT_FILENAME
        or screenshot["media_type"] != "image/png"
        or screenshot["width"] != SCREENSHOT_WIDTH
        or screenshot["height"] != SCREENSHOT_HEIGHT
        or screenshot["logical_width"] != SCREENSHOT_WIDTH
        or screenshot["logical_height"] != SCREENSHOT_HEIGHT
    ...
    if png_info["width"] != SCREENSHOT_WIDTH or png_info["height"] != SCREENSHOT_HEIGHT:
...
def _validate_schema3_manifest(
    ...
    if package_version is not None:
        validate_publication_version(package_version, manifest["version"], manifest["channel"])
    expected_build_id = f"gainsnap-v{manifest['version']}-{source['git_sha'][:12]}"
    ...
    expected_targets = {
        ("macos", "arm64", "clap"): f"gainsnap-v{manifest['version']}-macos.clap.zip",
        ("macos", "arm64", "vst3"): f"gainsnap-v{manifest['version']}-macos.vst3.zip",
        ("windows", "x86_64", "vst3"): f"gainsnap-v{manifest['version']}-windows-x86_64-unsigned.vst3.zip",
    }
    ...
    if digest != screenshot["sha256"] or size != screenshot["size_bytes"]:
        raise ValueError(f"manifest hash/size mismatch: {SCREENSHOT_FILENAME}")
...
def _validate_exact_manifest_names(manifest: Mapping[str, Any]) -> None:
    if manifest.get("schema_version") == MANIFEST_SCHEMA_V3:
        ...
        expected = {
            "clap-macos": f"gainsnap-v{version}-macos.clap.zip",
            "vst3-macos": f"gainsnap-v{version}-macos.vst3.zip",
            "vst3-windows": f"gainsnap-v{version}-windows-x86_64-unsigned.vst3.zip",
        }
        actual = {
        ...
        if actual != expected:
            raise ValueError("manifest artifact names do not match the exact GainSnap ZIP contract")
    else:
        channel = manifest.get("channel")
        version = manifest.get("version")
        expected = {"clap": f"gainsnap-v{version}-macos.clap.zip", "vst3": f"gainsnap-v{version}-macos.vst3.zip"}
        actual = {artifact.get("format"): artifact.get("name") for artifact in manifest.get("artifacts", [])}
        if actual != expected:
            raise ValueError("manifest artifact names do not match the exact GainSnap ZIP contract")
    if manifest.get("screenshot", {}).get("role") != "default-ui" or manifest.get("screenshot", {}).get("name") != SCREENSHOT_FILENAME or manifest.get("changelog", {}).get("name") != "CHANGELOG.md":
        raise ValueError("manifest support-file roles or names do not match the GainSnap release contract")
...
def validate_canonical_source(manifest: Mapping[str, Any], repo_root: Path) -> None:
    """Retain the original public name for canonical-source validation."""
    _validate_canonical_source(manifest, repo_root)
...
def _audit_zip(path: Path, format_name: str, expected_team: str, *, cwd: Path, require_production: bool = True) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="gainsnap-release-audit-") as temporary:
        extracted = Path(temporary)
        _run_checked(("/usr/bin/ditto", "-x", "-k", str(path), str(extracted)), cwd=cwd)
        bundle = extracted / f"gainsnap.{format_name}"
        ...
        if not all(path.exists() for path in required) or not (contents / "MacOS" / "gainsnap").is_file() or not os.access(contents / "MacOS" / "gainsnap", os.X_OK):
            raise ValueError(f"{format_name} ZIP bundle layout is invalid")
        info = contents / "Info.plist"
        binary = contents / "MacOS" / "gainsnap"
        ...
        if identifier != f"com.portalsurfer.gainsnap.{format_name}":
...
def publish_release(
    *,
    ...
    if manifest_path is not None:
        ...
        artifact_root = Path(root) if root is not None else Path.cwd()
        if not isinstance(manifest, Mapping):
    ...
    _publish_validated_manifest(endpoint=endpoint, token=token, manifest=manifest, root=artifact_root, transport=transport)
...
def validate_preflight_manifest(
    ...
    if manifest.get("distribution") != "preflight":
        raise ValueError("preflight requires ad hoc non-notarized provenance")
    if manifest.get("schema_version") != MANIFEST_SCHEMA_V2:
        raise ValueError("preflight manifests must use schema 2")
    validate_manifest(manifest, root, package_version=package_version)
    _validate_exact_manifest_names(manifest)
