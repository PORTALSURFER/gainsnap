#!/usr/bin/env python3
"""Update GainSnap's package and lockfile version for one release bump."""
...
def bump_package_version(manifest_path: Path, lock_path: Path, new_version: str) -> str:
    ...
    if lock_matches != 1:
        raise ValueError("Cargo.lock GainSnap package entry is missing or ambiguous")
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(f"bumped GainSnap from v{previous} to v{sys.argv[1]}")
    return 0
