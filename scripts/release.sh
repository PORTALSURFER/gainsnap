...
Options:
  ...
  --build-id ID                Immutable id (default: gainsnap-v<publication-version>-<12-char source SHA>)
...
build_id="${build_id:-gainsnap-v${publication_version}-${source_sha:0:12}}"
[[ "${build_id}" =~ ^[a-z0-9][a-z0-9._-]{1,127}$ ]] || { echo "invalid build id" >&2; exit 2; }
if [[ "${mode}" != preflight && "${channel}" == nightly ]]; then
  expected_build_id="gainsnap-v${publication_version}-${source_sha:0:12}"
...
expected_archive = f"gainsnap-v{publication_version}-windows-x86_64-unsigned.vst3.zip"
expected_build_id = f"gainsnap-v{publication_version}-{source_sha[:12]}"
source = manifest.get("source")
if (manifest.get("schema_version") != 1 or manifest.get("product") != "gainsnap" or manifest.get("format") != "vst3" or manifest.get("platform") != "windows" or manifest.get("architecture") != "x86_64" or manifest.get("package_version") != package_version or manifest.get("publication_version") != publication_version or manifest.get("channel") != "nightly" or manifest.get("build_id") != expected_build_id or manifest.get("build_id") != build_id or manifest.get("released_at") != released_at or not isinstance(source, dict) or source.get("git_sha") != source_sha or source.get("repository") != "PORTALSURFER/gainsnap" or source.get("dirty") is not False or manifest.get("signing_status") != "unsigned" or manifest.get("signing_certificate") is not None or not isinstance(manifest.get("archive"), dict) or manifest["archive"].get("name") != expected_archive):
...
PY
  windows_archive_name="gainsnap-v${publication_version}-windows-x86_64-unsigned.vst3.zip"
fi

release_parent="${repo_root}/dist/releases"
release_dir="${release_parent}/${build_id}"
[[ ! -e "${release_dir}" ]] || { echo "release already exists locally: ${release_dir}" >&2; exit 1; }
...
if [[ -n "${windows_archive_name}" ]]; then
  cp "${windows_release_root}/${windows_archive_name}" "${staged}/${windows_archive_name}"
fi
...
echo "[release] rendering GainSnap screenshots"
...
png="$(find target/ui-screenshots -type f -name "*${screenshot_width}x${screenshot_height}*.png" -print | head -n 1)"
cp "${png}" "${staged}/${screenshot_name}"

build_bundle() {
  local format="$1" target_zip="$2" bundle_dir="$3" binary="$4"
  local contents="${bundle_dir}/Contents"
  mkdir -p "${contents}/MacOS"
  cp "${binary}" "${contents}/MacOS/gainsnap"
  chmod 755 "${contents}/MacOS/gainsnap"
  cat > "${contents}/Info.plist" <<EOF
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
<key>CFBundleExecutable</key><string>gainsnap</string>
<key>CFBundleIdentifier</key><string>com.portalsurfer.gainsnap.${format}</string>
<key>CFBundleName</key><string>GainSnap</string>
...
PY
  ...
  file "${contents}/MacOS/gainsnap" | grep -q arm64 || { echo "${format} binary is not arm64" >&2; exit 1; }
  if command -v lipo >/dev/null 2>&1; then [[ "$(lipo -archs "${contents}/MacOS/gainsnap")" == arm64 ]] || { echo "${format} binary must contain only arm64" >&2; exit 1; }; fi
  if [[ "${format}" == clap ]]; then /usr/bin/nm -gU "${contents}/MacOS/gainsnap" | grep -q _clap_entry || { echo "CLAP entrypoint missing" >&2; exit 1; }; else
    for symbol in _GetPluginFactory _bundleEntry _bundleExit; do /usr/bin/nm -gU "${contents}/MacOS/gainsnap" | grep -q "${symbol}" || { echo "VST3 symbol ${symbol} missing" >&2; exit 1; }; done
  fi
...
build_bundle clap "${staged}/gainsnap-v${publication_version}-macos.clap.zip" "${tmp_root}/gainsnap.clap" "${clap_binary}"
audit_zip clap "${staged}/gainsnap-v${publication_version}-macos.clap.zip"
...
vst3_binary="${vst3_target}/release/libgainsnap.dylib"
[[ -f "${vst3_binary}" ]] || { echo "VST3 build did not produce ${vst3_binary}" >&2; exit 1; }
build_bundle vst3 "${staged}/gainsnap-v${publication_version}-macos.vst3.zip" "${tmp_root}/gainsnap.vst3" "${vst3_binary}"
...
kwargs = dict(publication_version=publication, package_version=package, build_id=build, channel=channel, released_at=released_at, git_sha=source, clap=folder / f"gainsnap-v{publication}-macos.clap.zip", vst3=folder / f"gainsnap-v{publication}-macos.vst3.zip", screenshot=folder / "gainsnap-default-300x320.png", changelog=folder / "CHANGELOG.md")
if mode == "preflight":
