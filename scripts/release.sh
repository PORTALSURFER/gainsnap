...
screenshot_count="$(find target/ui-screenshots -type f -name "initial-ui-${screenshot_width}x${screenshot_height}.png" -print | wc -l | tr -d ' ')"
[[ "${screenshot_count}" == 1 ]] || { echo "release requires exactly one fresh ${screenshot_width}x${screenshot_height} screenshot" >&2; exit 1; }
png="$(find target/ui-screenshots -type f -name "initial-ui-${screenshot_width}x${screenshot_height}.png" -print | head -n 1)"
